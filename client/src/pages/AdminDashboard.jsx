...
function Field({ label, children }) {
  return <label className="block"><span className="text-sm text-gray-400 block mb-1">{label}</span>{children}</label>
}
function StatCard({ icon, label, value }) {
  return <div className="rounded-xl border border-dark-border bg-dark-card p-4"><div>{icon}</div><p className="text-sm text-gray-400 mt-2">{label}</p><p className="text-3xl font-bold">{value}</p></div>
}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 p-4 flex items-center justify-center">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-dark-border bg-dark-card p-5">
        <div className="flex items-center justify-between mb-4"><h3 className="text-xl font-semibold">{title}</h3><button onClick={onClose} className="cursor-pointer">✕</button></div>
        {children}
      </div>
    </div>
  )
